                        ...
                        {formData.scoringConfig.taskTypes?.includes('Dimensional Scoring (1-5)') ? (
                          <>
                            ...
                            <div className="flex flex-col gap-2">
                              {formData.scoringConfig.scoreDimensions.map((dim, i) => (
                                <div key={i} className="flex items-center gap-2 animate-in fade-in duration-150">
                                  <input
                                    ...
                                    value={dim}
                                    onChange={e => {
                                      const oldName = formData.scoringConfig.scoreDimensions[i];
                                      const newName = e.target.value;
                                      const newD = [...formData.scoringConfig.scoreDimensions];
                                      newD[i] = newName;

                                      // Rename key in scoringAnchors to preserve entered description
                                      const oldAnchors = (formData.scoringConfig as any).scoringAnchors || {};
                                      const newAnchors = { ...oldAnchors };
                                      if (oldName && oldName !== newName && newAnchors[oldName]) {
                                        newAnchors[newName] = newAnchors[oldName];
                                        delete newAnchors[oldName];
                                      }
                                      ...
                                      setFormData({
                                        ...
                                        scoringConfig: {
                                          ...
                                          scoreDimensions: newD,
                                          scoringAnchors: newAnchors
                                        } as any
                                      });
                                    }}
                                    placeholder="Dimension name (e.g. Fluency)"
                                    className="bg-zinc-950 border border-zinc-800 p-2 text-zinc-300 text-xs outline-none flex-1 focus:border-indigo-500 transition-colors"
                                  />
                                  <div className="flex items-center gap-1">
                                    ...
                                    <input
                                      ...
                                      className="bg-zinc-950 border border-zinc-800 p-2 text-zinc-300 text-xs outline-none w-14 focus:border-indigo-500 transition-colors"
                                    />
                                  </div>
                                  <button type="button" onClick={() => {
                                    ...
                                    // Clean up the key in scoringAnchors
                                    const oldAnchors = (formData.scoringConfig as any).scoringAnchors || {};
                                    const newAnchors = { ...oldAnchors };
                                    if (oldName) {
                                      delete newAnchors[oldName];
                                    ...
                                    setFormData({
                                      ...
                                      scoringConfig: {
                                        ...formData.scoringConfig,
                                        scoreDimensions: newD,
                                        dimensionWeights: newW,
                                        scoringAnchors: newAnchors
                                      } as any
                                    });
                                  }} className="text-zinc-600 hover:text-red-500 p-1"><X size={14} /></button>
                                </div>
                              ))}
                              ...
                              })} className="text-[10px] text-left text-indigo-400 mt-2 hover:text-indigo-300 font-mono font-bold">+ Add Custom Dimension</button>
                            </div>
                          </>
                        ) : (
                          <div className="h-full flex flex-col justify-center items-center border border-dashed border-zinc-800/80 p-6 text-center bg-black/40">
                            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest mb-1">Dimensional Scoring Disabled</span>
                            <p className="text-[9px] text-zinc-650 font-sans max-w-xs">Enable "Dimensional Scoring (1-5)" on the left to configure custom dimensions & weights.</p>
                          </div>
                        )}
                      </div>

                      <div className="space-y-2 col-span-2 pt-6 border-t border-zinc-900 grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                        ...
                        <div>
                          ...
                          <input type="number" value={formData.scoringConfig.minLength} onChange={e => setFormData({ ...formData, scoringConfig: { ...formData.scoringConfig, minLength: parseInt(e.target.value) || 0 } })} className="bg-zinc-950 border border-zinc-800 p-2 text-zinc-300 text-sm outline-none w-24 focus:border-indigo-500 transition-colors" /> chars
                        </div>
                        {formData.scoringConfig.taskTypes?.includes('Preference Ranking (A vs B)') ? (
                        ...
                        {formData.scoringConfig.taskTypes?.includes('Preference Ranking (A vs B)') && formData.scoringConfig.allowTie && (
                          <div className="col-span-1 md:col-span-3 pt-4 border-t border-zinc-900 animate-in slide-in-from-top-2 duration-200">
