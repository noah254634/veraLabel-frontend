...
const handleSearch = async (e) => {
  ...
  if (value.length > 2) {
    try {
      // Dynamic backend call based on the selected dropdown type
      const response = await fetch(`/api/users/search?${searchType}=${value}`);
    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-white-100 text-sm">
            Review, Moderate, and Manage Global Data Assets
          </p>
        </div>
      ...
      <div className="bg-white p-2 rounded-2xl border border-slate-200 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 flex-1 px-2">
          <input
            type="text"
            placeholder={`Sort by ${searchType} `}
      ...
      <div className="flex-1 flex gap-6 overflow-hidden min-h-[600px]">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-gray-900">
                ...
                <th className="p-4 text-[11px] font-black uppercase text-slate-400 tracking-wider text-right">
                  Actions
                </th>
              </tr>
            </thead>
        ...
        {/* RIGHT: ACTION PANEL */}
        <aside
          className={`w-96 bg-slate-900 rounded-3xl p-6 text-white transition-all transform ${selectedDataset ? "translate-x-0 opacity-100" : "translate-x-10 opacity-0"}`}
        >
          {selectedDataset ? (
            <div className="flex flex-col h-full">
              ...
              <p className="text-slate-400 text-xs mb-6 font-mono">
                {selectedDataset.datasetId ?? String(selectedDataset._id)}
              </p>
              ...
              <div className="space-y-4 flex-1">
                <div className="bg-slate-800/50 p-4 rounded-2xl border border-slate-700/50">
                  ...
                  <p className="text-sm text-slate-300 italic">
                    No red flags detected by automated filters. Ready for human
                    review.
                  </p>
                </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center px-6">
              ...
              <p className="text-slate-600 text-xs mt-2">
                Select an item from the inventory to view metadata and perform
